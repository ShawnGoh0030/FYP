...
def sampling(QPRu_list, initial, QPRm):
    ...
    while samples < samples_needed:
        ...
        if len(QPRm) == len(initial):
            p_traj *= QPRm[index[-1]]
        else:
            p_traj *= QPRm[ math.floor(index[-1]/4) ]  #For qubit Grover algo
...
def new_sampling(QPRu_list, initial, QPRm):
    ...
    for i in range(length):
        if initial[i] == 0:
            ...
            entries.append((total_neg, i, initial[i] / total_neg))
    
    #each QPRu
    curr_terms = entries
    for QPRu in tqdm(QPRu_list):
        ...
        for term in curr_terms:
            ...
            for i in range(length):
                ...
                if temp == 0:
                    ...
                    #print(term[2]*(temp/col_neg))
                    #print((col_neg*term[0]*sign(temp)))
                    next_terms.append( (col_neg*term[0], i, term[2]*(temp/col_neg)) )
    ...
    for entry in curr_terms:
        ...
        values.append( entry[0] * sign(entry[2]) * QPRm[entry[1]] )
    #print(values)
    #print(sum(probabilities))
    #print(len(probabilities))
    start = time.time()
    rng = np.random.default_rng()
    samples = rng.choice(values, size = samples_needed, p = probabilities)
    end = time.time()
    time_taken = end - start
    p = np.sum(samples)
    return (p/samples_needed, samples_needed, time_taken)
'''
#Testing
FG = wig.DW(2)
frame = FG[:4]
